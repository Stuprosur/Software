...
int main(int argc, char **argv) {
  ...
  // 创建一个名为 `scout` 的智能指针，该指针指向 `ScoutMiniOmniRobot` 类的实例
  std::unique_ptr<ScoutMiniOmniRobot> scout;

  // 创建 ProtocolDetector 对象并连接到指定的设备
  ProtocolDetector detector;
  detector.Connect(device_name);
  auto proto = detector.DetectProtocolVersion(5);


  // 根据协议版本创建 ScoutMiniOmniRobot 实例
  ...
  while (true) {
    ...
    // 打印小车状态信息，如控制模式、车辆状态、错误码、电池电压、速度等
    std::cout << "-------------------------------" << std::endl;
